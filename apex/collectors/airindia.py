"""Air India direct carrier response parser and collector adapter."""
...
class AirIndiaResponseParser:
    """Parser for Air India search responses and API payloads."""
    ...
    def parse(
        ...
        run_identifier = (
            run_id or f"run-airindia-{travel_date.strftime('%Y%m%d')}-{origin}-{destination}"
        )
        ...
        elif "data" in data and isinstance(data["data"], dict) and "journeys" in data["data"]:
            for j in data["data"]["journeys"]:
                flights.extend(j.get("flights", []))
        ...
        for flight_data in flights:
            try:
                ...
                for fare_entry in fares:
                    ...
                    obs_id = (
                        f"obs-ai-{flight_number.lower().replace('-', '')}-"
                ...
                logger.warning("Skipping Air India flight entry: %s", e)
                continue

        return observations


class AirIndiaCollector(BaseCollector):
    """Air India direct airline scraper adapter with circuit-breaker protection."""

    def __init__(
        self,
        name: str = "AirIndiaCollector",
        source_code: str = "airindia_direct",
        ...
        self.parser = AirIndiaResponseParser(non_stop_only=non_stop_only)
        self.transport = transport

    async def collect_route(
        self, origin: str, destination: str, travel_date: date, window_label: str
    ) -> CollectorResult:
        """Collect and parse Air India route observations."""
                        ...
                        "flights": [
                            {
                                "flightNumber": "AI-805",
                                "carrierCode": "AI",
                                "departureTime": f"{travel_date.isoformat()}T08:00:00Z",
                                "arrivalTime": f"{travel_date.isoformat()}T10:15:00Z",
                                "stops": 0,
                                "isNonStop": True,
                                "fares": [
                                    {
                                        "fareFamily": "Eco Value",
                                        "baseFare": 4200.0,
                                        "taxes": 520.0,
                                        "fees": 282.0,
                                        "totalFare": 5002.0,
