"""SpiceJet direct carrier response parser and collector adapter."""
...
class SpiceJetResponseParser:
    """Parser for SpiceJet flight search responses."""
    ...
    def parse(
        ...
        run_identifier = (
            run_id or f"run-spicejet-{travel_date.strftime('%Y%m%d')}-{origin}-{destination}"
        )
        ...
        for flight_data in flights:
            try:
                ...
                for fare_entry in fares:
                    ...
                    obs_id = (
                        f"obs-sg-{flight_number.lower().replace('-', '')}-"
                ...
                logger.warning("Skipping SpiceJet flight entry: %s", e)
                continue

        return observations


class SpiceJetCollector(BaseCollector):
    """SpiceJet direct airline scraper adapter with circuit-breaker protection."""

    def __init__(
        self,
        name: str = "SpiceJetCollector",
        source_code: str = "spicejet_direct",
        ...
        self.parser = SpiceJetResponseParser(non_stop_only=non_stop_only)
        self.transport = transport

    async def collect_route(
        self, origin: str, destination: str, travel_date: date, window_label: str
    ) -> CollectorResult:
        """Collect and parse SpiceJet route observations."""
                        ...
                        "flights": [
                            {
                                ...
                                "carrierCode": "SG",
                                "departureTime": f"{travel_date.isoformat()}T06:30:00Z",
                                "arrivalTime": f"{travel_date.isoformat()}T08:50:00Z",
                                "stops": 0,
                                "isNonStop": True,
                                "fares": [
                                    {
