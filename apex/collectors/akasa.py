"""Akasa Air direct carrier response parser and collector adapter."""
...
class AkasaResponseParser:
    """Parser for Akasa Air flight search response payloads."""
    ...
    def parse(
        ...
        run_identifier = (
            run_id or f"run-akasa-{travel_date.strftime('%Y%m%d')}-{origin}-{destination}"
        )
        ...
        for flight_data in flights:
            try:
                ...
                for fare_entry in fares:
                    ...
                    obs_id = (
                        f"obs-qp-{flight_number.lower().replace('-', '')}-"
                ...
                logger.warning("Skipping Akasa flight entry: %s", e)
                continue

        return observations


class AkasaCollector(BaseCollector):
    """Akasa Air direct airline scraper adapter with circuit-breaker protection."""

    def __init__(
        self,
        name: str = "AkasaCollector",
        source_code: str = "akasa_direct",
        ...
        self.parser = AkasaResponseParser(non_stop_only=non_stop_only)
        self.transport = transport

    async def collect_route(
        self, origin: str, destination: str, travel_date: date, window_label: str
    ) -> CollectorResult:
        """Collect and parse Akasa route observations."""
                        ...
                        "flights": [
                            {
                                ...
                                "carrierCode": "QP",
                                "departureTime": f"{travel_date.isoformat()}T07:15:00Z",
                                "arrivalTime": f"{travel_date.isoformat()}T09:30:00Z",
                                "stops": 0,
                                "isNonStop": True,
                                "fares": [
                                    {
