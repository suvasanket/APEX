"""Unit tests for AirIndiaResponseParser and AirIndiaCollector."""
...
SAMPLE_FIXTURE_PATH = (
    ...
    / "fixtures"
    / "observations"
    / "airindia_response_sample.json"
)


class TestAirIndiaParser(unittest.TestCase):
    """Test suite for Air India response parser and collector."""

    def setUp(self):
        self.parser = AirIndiaResponseParser(non_stop_only=True)
        with open(SAMPLE_FIXTURE_PATH, "r", encoding="utf-8") as f:
            self.sample_payload = f.read()

    def test_normalize_flight_number(self):
        self.assertEqual(AirIndiaResponseParser.normalize_flight_number("AI-805"), "AI-805")
        self.assertEqual(AirIndiaResponseParser.normalize_flight_number("AI805"), "AI-805")
        self.assertEqual(AirIndiaResponseParser.normalize_flight_number("805"), "AI-805")
        self.assertEqual(AirIndiaResponseParser.normalize_flight_number("  ai-102 "), "AI-102")
    ...
    def test_parse_sample_fixture_success(self):
        ...
        # AI-805 has 2 fares (Eco Value, Eco Flex), AI-887 has 1 fare (Eco Value).
        # AI-441 has 1 stop, filtered out. Total = 3 observations.
        self.assertEqual(len(observations), 3)
        flight_numbers = [obs.flight_identity.flight_number for obs in observations]
        self.assertIn("AI-805", flight_numbers)
        self.assertIn("AI-887", flight_numbers)
        self.assertNotIn("AI-441", flight_numbers)

        for obs in observations:
            self.assertIsInstance(obs, FareObservation)
            self.assertEqual(obs.flight_identity.airline_iata, "AI")
    ...
    def test_airindia_collector_execution(self):
        collector = AirIndiaCollector()
        ...
        try:
            ...
            self.assertEqual(res.observations[0].flight_identity.airline_iata, "AI")
